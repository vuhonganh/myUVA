...
struct team
{
  ...
  void process(int teamNb, char probLetter, int hour, int minute, char status)
  {
    m_teamNb = teamNb;
	...
	if (m_ProbSubm[probLetter][0] != -1) //already solved
	  {
	    if (status == 'N')
	      {
		if (curTime < m_ProbSubm[probLetter][0]) m_time += 20;
	      }
	    else //already solved - check which one is the sooner
	      {
